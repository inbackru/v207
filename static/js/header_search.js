...
class SmartSearch {
    ...
    initPropertySearch() {
        if (this.propertySearchInput && this.propertyDropdown) {
            // Skip if SuperSearch already manages this input — it handles novostrojki suggestions
            if (this.propertySearchInput.dataset.superSearchInitialized) {
                console.log('⏭️ Skipping header SmartSearch for property-search — SuperSearch handles it');
                return;
            }
...
SmartSearch.prototype.renderModernSuggestions = function(suggestions, dropdown) {
    ...
    function getSuggestionBadge(s) {
        if (s.type === 'district' && s.district_type) {
            return districtTypeNames[s.district_type] || 'Район';
        }
        return typeNames[s.type] || '';
    }
    ...
    dropdown.querySelectorAll('.suggestion-item').forEach(item => {
        item.addEventListener('click', () => {
            ...
            // ЖК или Застройщик на странице новостроек — применяем фильтр, НЕ переходим
            const isNovostrojkiPage = /\/novostrojki/.test(window.location.pathname);
            if (isNovostrojkiPage && (type === 'complex' || type === 'developer') && value) {
                // Скрываем dropdown
                dropdown.classList.add('hidden');
                // Вызываем единый обработчик выбора (определён в properties.html)
                if (typeof window.handlePropertySuggestionSelect === 'function') {
                    window.handlePropertySuggestionSelect({ type: type, text: value, url: rawUrl });
                } else if (typeof handlePropertiesSearchSelection === 'function') {
                    handlePropertiesSearchSelection({ type: type, text: value, url: rawUrl });
                }
                return;
            }
