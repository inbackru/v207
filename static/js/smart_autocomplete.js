...
class SmartAutocomplete {
        ...
        const html = suggestions.map((suggestion, index) => {
            ...
            <div class="suggestion-item ${index === this.selectedIndex ? 'selected' : ''}" data-index="${index}">
                ...
                <div class="suggestion-type">
                    ${this.getTypeLabel(suggestion.type, suggestion.district_type)}
                </div>
            </div>`;
        }).join('');
    ...
    getTypeLabel(type, districtType) {
        ...
        if (type === 'district' && districtType) {
            return districtTypeNames[districtType] || 'Район';
        }
        const labels = {
            'address': 'Адрес',
            'complex': 'ЖК',
    ...
    selectSuggestion(index) {
        ...
        if (index >= 0 && index < this.suggestions.length) {
            ...
            // ЖК и Застройщик — всегда применяем фильтр через onSelect, НЕ навигируем
            if (suggestion.type === 'complex' || suggestion.type === 'developer') {
                this.input.value = '';
                this.hideSuggestions();
            ...
            // Для остальных типов (город, район, адрес, тип квартиры) — переходим по URL
            if (suggestion.url) {
