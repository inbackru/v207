...
// Notification click + action button handling
self.addEventListener('notificationclick', event => {
    ...
    // Resolve destination URL based on action button or tag
    let targetUrl = data.url || '/';
    if (action === 'open_chat'    || tag.startsWith('chat-') || tag.startsWith('mgr-chat-')) {
        targetUrl = data.url || '/dashboard?tab=chat';
    } else if (action === 'view_deal'    ) { targetUrl = '/dashboard#deals';        }
    else if (action === 'view_balance'   ) { targetUrl = '/dashboard#cashback';     }
    else if (action === 'view_apps'      ) { targetUrl = '/dashboard#applications'; }
    else if (action === 'view_results'   ) { targetUrl = data.url || '/dashboard#saved-searches'; }
    else if (action === 'view_complex'   ) { targetUrl = data.url || '/';           }
    else if (action === 'view_lead'      ) { targetUrl = '/manager/clients';        }

    const isChatNotif = tag.startsWith('chat-') || tag.startsWith('mgr-chat-') || action === 'open_chat';

    event.waitUntil(
        clients.matchAll({ type: 'window', includeUncontrolled: true }).then(function(windowClients) {
            /* Prefer existing tab — focus it and post a message to open the right panel */
            for (var i = 0; i < windowClients.length; i++) {
                var client = windowClients[i];
                if (client.url.startsWith(self.location.origin)) {
                    client.focus();
                    client.postMessage({
                        type: isChatNotif ? 'OPEN_CHAT' : 'NAVIGATE',
                        url: targetUrl,
                        tag: tag,
                        action: action,
                    });
            ...
            /* No existing tab — open new window with target URL */
