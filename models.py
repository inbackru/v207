...
class City(db.Model):
    ...
    @property
    def name_genitive_safe(self):
        """Родительный падеж: «Краснодара», «Сочи». Возвращает name_genitive или name как запасной вариант."""
        return self.name_genitive or self.name

    @property
    def name_prepositional_safe(self):
        """Предложный падеж: «Краснодаре», «Сочи». Возвращает name_prepositional или name как запасной вариант."""
        return self.name_prepositional or self.name
...
class PersonalCollection(db.Model):
    """Личные подборки, создаваемые клиентами самостоятельно"""
    ...
    user = db.relationship('User', backref='personal_collections')
    items = db.relationship('PersonalCollectionItem', backref='collection', cascade='all, delete-orphan')
    ...
    def to_dict(self):
        return {
            ...
            'title': self.title,
            'description': self.description,
            'properties_count': len(self.items),
            'created_at': self.created_at.strftime('%d.%m.%Y') if self.created_at else ''
        }


class PersonalCollectionItem(db.Model):
    """Объект в личной подборке клиента"""
    __tablename__ = 'personal_collection_items'
    __table_args__ = {"extend_existing": True}

    id = db.Column(db.Integer, primary_key=True)
    collection_id = db.Column(db.Integer, db.ForeignKey('personal_collections.id'), nullable=False)
...
class ResidentialComplex(db.Model):
    ...
    # okrug_district_id: the parent okrug (округ) this complex belongs to,
    # populated via PIP against okrug polygons. Used for aggregated okrug filtering.
    okrug_district_id = db.Column(db.Integer, db.ForeignKey('districts.id'), nullable=True)
    ...
    security_type = db.Column(db.String(200), nullable=True)   # Устаревшее: строка охраны (см. security_features)
    has_concierge = db.Column(db.Boolean, nullable=True)       # Есть ли консьерж
    lifts_count = db.Column(db.Integer, nullable=True)         # Минимальное кол-во лифтов
    lifts_range = db.Column(db.String(50), nullable=True)      # Диапазон лифтов: "от 2 до 10"
    ...
    delivery_range = db.Column(db.String(50), nullable=True)   # Диапазон сдачи: "2021-2026"
    territory_amenities = db.Column(db.Text, nullable=True)    # JSON: объекты на территории
    parking_features = db.Column(db.Text, nullable=True)       # JSON: список типов парковок
    security_features = db.Column(db.Text, nullable=True)      # JSON: полный список охраны
    delivery_schedule = db.Column(db.Text, nullable=True)      # JSON: сдача по корпусам [{name, quarter, year}]
    ...
    boundary_geometry = db.Column(db.Text, nullable=True)      # OSM building polygon: "lat,lng;lat,lng;..."
    
    # CIAN-parsed address fields (auto-populated from address field by utils/address_parser.py)
    address_quarter = db.Column(db.String(200), nullable=True)        # мкр/микрорайон (e.g. "Горхутор мкр")
    address_city_district = db.Column(db.String(200), nullable=True)  # жилрайон/округ (e.g. "Прикубанский округ")
    addr_region = db.Column(db.String(200), nullable=True)            # Краснодарский край, Курская область...
    addr_city = db.Column(db.String(100), nullable=True)              # Краснодар, Сочи, Майкоп...
    addr_street = db.Column(db.String(300), nullable=True)            # улица Агрономическая, проспект Чекистов...
    addr_house = db.Column(db.String(50), nullable=True)              # 13/9, 87к1, 2/1...

    # Aggregated stats (from TrendAgent)
    apartments_count = db.Column(db.Integer, nullable=True)           # Общее число квартир (apartCount)
    parking_count = db.Column(db.Integer, nullable=True)              # Число машиномест
    commerce_count = db.Column(db.Integer, nullable=True)             # Число коммерческих помещений

    # TrendAgent extended fields
    pano_url = db.Column(db.String(500), nullable=True)               # URL 3D-панорамы
    ta_level_type = db.Column(db.String(100), nullable=True)          # Класс ЖК: Бизнес / Комфорт / Эконом
    ta_location_name = db.Column(db.String(200), nullable=True)       # Район/локация TA ("ЧМР", "Фестивальный")
    ta_location_guid = db.Column(db.String(200), nullable=True)       # GUID локации TA
    facade_type = db.Column(db.String(100), nullable=True)            # Тип фасада (Облицовочный кирпич, ...)
    contract_type = db.Column(db.String(100), nullable=True)          # Тип договора (ДДУ, КП, ...)
    ta_payment_types = db.Column(db.Text, nullable=True)              # JSON: ["Рассрочка","Ипотека",...]
    is_exclusive = db.Column(db.Boolean, nullable=True)               # Эксклюзивный ЖК
    is_mortgage_available = db.Column(db.Boolean, nullable=True)      # Ипотека доступна
    view_places = db.Column(db.String(300), nullable=True)            # Видовые квартиры
    interactive_plan_url = db.Column(db.String(500), nullable=True)   # URL интерактивного плана
    has_gym = db.Column(db.Boolean, nullable=True)                    # Есть фитнес-зал
    has_garden = db.Column(db.Boolean, nullable=True)                 # Есть сад/парк
    ta_rooms_types = db.Column(db.Text, nullable=True)                # JSON: типы комнат в ЖК [0,1,2,22,...]
    ta_passport = db.Column(db.Text, nullable=True)                   # JSON: паспорт ЖК от TA
    ta_min_price = db.Column(db.Integer, nullable=True)               # Мин. цена из TA (minPrice)

    # TrendAgent v2 extended fields (шахматка-эпоха)
    ta_crm_id = db.Column(db.BigInteger, nullable=True)               # CRM ID ЖК в системе TA (64-bit)
    ta_guid = db.Column(db.String(200), nullable=True)                # GUID/slug ЖК в TA (garantiya-na-obryvnoy)
    ta_advantage = db.Column(db.Text, nullable=True)                  # HTML-текст преимуществ ЖК (advantage)
    ta_subways = db.Column(db.Text, nullable=True)                    # JSON: станции метро [{name,distance_type,time}]
    ta_point_distances = db.Column(db.Text, nullable=True)            # JSON: расстояния до ключевых точек
    ta_status = db.Column(db.Integer, nullable=True)                  # Статус ЖК в TA (1=активный)
    ta_aliases = db.Column(db.Text, nullable=True)                    # JSON: список псевдонимов ЖК (до 53+)
    is_with_prayer_room = db.Column(db.Boolean, nullable=True)        # Есть молельная комната
    ta_plan_images = db.Column(db.Text, nullable=True)                # JSON: генпланы/планировки [{src,title}]
    ta_reward_label = db.Column(db.String(100), nullable=True)        # Вознаграждение агентам "4.5-6.5%" (уровень ЖК)
    ta_aerial_panorama_url = db.Column(db.String(500), nullable=True) # URL аэропанорамы (aerial/drone)
    ta_sales_start_display = db.Column(db.String(200), nullable=True) # Старт продаж (из паспорта TA) — "Апрель 2023"
    ta_escrow = db.Column(db.Boolean, nullable=True)                  # Эскроу доступно (из паспорта)
    ta_deadline_key_display = db.Column(db.String(200), nullable=True)# Срок передачи ключей — "Заселен" / "1 кв. 2025"
    ta_renderer_json = db.Column(db.Text, nullable=True)              # JSON: полный renderer с titles/alt/desc
    ta_point_metro_time = db.Column(db.Integer, nullable=True)        # Время до ближайшей станции метро (мин)
    ta_min_area = db.Column(db.Float, nullable=True)                  # Мин. площадь квартир (из шахматки)
    ta_max_area = db.Column(db.Float, nullable=True)                  # Макс. площадь квартир (из шахматки)
    ta_min_floor = db.Column(db.Integer, nullable=True)               # Мин. этаж (из шахматки)
    ta_max_floor = db.Column(db.Integer, nullable=True)               # Макс. этаж (из шахматки)
    ta_scraped_at = db.Column(db.DateTime, nullable=True)             # Дата последнего обогащения из TA
    parsed_at = db.Column(db.DateTime, nullable=True)                 # Алиас: та же дата для единообразия
    parsing_status = db.Column(db.String(50), nullable=True)          # Статус парсинга: success / error / partial
    ...
    # Relationships
    district = db.relationship('District', foreign_keys=[district_id], backref='complexes')
    okrug_district = db.relationship('District', foreign_keys=[okrug_district_id], backref='okrug_complexes')
    ...
    @property
    def cian_address(self):
        """ЦИАН-стиль: Краснодарский край → Краснодар → Прикубанский → ул. Ленина, 10"""
        parts = []
        try:
    ...
    @property
    def cian_address_parts(self):
        """Словарь с компонентами адреса в ЦИАН-стиле"""
        return {
            'region': (self.city.region.name if self.city and self.city.region else ''),
...
class Building(db.Model):
    ...
    end_build_year = db.Column(db.Integer, nullable=True)
    end_build_quarter = db.Column(db.Integer, nullable=True)
    deadline_key = db.Column(db.DateTime, nullable=True)         # Дата передачи ключей
    sales_start_at = db.Column(db.DateTime, nullable=True)       # Старт продаж корпуса
    queue = db.Column(db.Integer, nullable=True)                 # Номер очереди строительства

    # Additional info
    complex_product = db.Column(db.String(100), nullable=True)   # тип продукта
    building_type_name = db.Column(db.String(100), nullable=True) # Тип здания (Кирпично-монолитный, ...)
    facade_type_name = db.Column(db.String(100), nullable=True)  # Тип фасада
    elevator_type = db.Column(db.String(100), nullable=True)     # Тип лифта
    address_street = db.Column(db.String(300), nullable=True)    # Адрес корпуса

    # TrendAgent flags
    escrow = db.Column(db.Boolean, nullable=True)                # Эскроу-счёт
    has_mortgage = db.Column(db.Boolean, nullable=True)          # Ипотека
    has_installment = db.Column(db.Boolean, nullable=True)       # Рассрочка
    has_subsidy = db.Column(db.Boolean, nullable=True)           # Субсидия
    has_mortgage_military = db.Column(db.Boolean, nullable=True) # Военная ипотека
    is_exclusive = db.Column(db.Boolean, nullable=True)          # Эксклюзивный корпус

    # TrendAgent JSON lists per building
    parking_types = db.Column(db.Text, nullable=True)            # JSON: ["Стоянка","Подземный паркинг"]
    finishing_types = db.Column(db.Text, nullable=True)          # JSON: ["Подчистовая","Чистовая дозаказ"]
    contract_types = db.Column(db.Text, nullable=True)           # JSON: ["КП"]
    ta_payment_types = db.Column(db.Text, nullable=True)         # JSON: ["100% оплата","Рассрочка","Ипотека"]

    # Statistics
    total_floors = db.Column(db.Integer, nullable=True)
    total_apartments = db.Column(db.Integer, nullable=True)

    # TrendAgent v2 extended building fields
    escrow_banks = db.Column(db.Text, nullable=True)             # JSON: список банков-эскроу ["Сбербанк"]
    ta_permit = db.Column(db.String(200), nullable=True)         # Разрешение на строительство (trakheesi_permit)
    apartment_count = db.Column(db.Integer, nullable=True)       # Кол-во квартир в корпусе (building.apartment_count)
    is_with_pool = db.Column(db.Boolean, nullable=True)          # Есть бассейн
    pool_types = db.Column(db.Text, nullable=True)               # JSON: типы бассейнов
    safety_types = db.Column(db.Text, nullable=True)             # JSON: охрана/безопасность
    deadline_key_type = db.Column(db.Integer, nullable=True)     # Тип даты передачи ключей (1=квартал,2=точная)
    subsidy = db.Column(db.Boolean, nullable=True)               # Субсидированная ипотека
    ta_renderer = db.Column(db.Text, nullable=True)              # JSON: рендеры корпуса [{path,file_name,title}]
    ta_interactive_geometry = db.Column(db.Text, nullable=True)  # JSON: интерактивная геометрия корпуса
    ta_scraped_at = db.Column(db.DateTime, nullable=True)        # Дата последнего обогащения корпуса
    boundary_geometry = db.Column(db.Text, nullable=True)        # Полигон корпуса: "lat,lng;lat,lng"
...
class Property(db.Model):
    ...
    gallery_images = db.Column(db.Text, nullable=True)  # JSON array of image URLs
    plan_image = db.Column(db.String(500), nullable=True)        # Планировка квартиры (схематичный чертёж)
    floor_plan_image = db.Column(db.String(500), nullable=True)  # План этажа (расположение квартиры на этаже)
    ...
    balcony_type = db.Column(db.String(50), nullable=True)        # Тип балкона: Лоджия / Балкон / Терраса
    window_type = db.Column(db.String(50), nullable=True)         # Тип окон: Стандартные / Увеличенные
    ...
    area_given = db.Column(db.Float, nullable=True)               # S приведенная (для расчёта цены)
    price_full_payment = db.Column(db.Integer, nullable=True)     # Цена при 100% оплате
    ta_start_price = db.Column(db.Integer, nullable=True)         # Стартовая цена из TA
    ta_start_price_sqm = db.Column(db.Integer, nullable=True)     # Стартовая цена за м² из TA

    # TrendAgent apartment (шахматка) fields
    is_suite = db.Column(db.Boolean, nullable=True)               # Апартамент-сьют
    ta_view_type = db.Column(db.Integer, nullable=True)           # Тип вида из окна (код TA: 1=видовая)
    ta_view_places = db.Column(db.Text, nullable=True)            # JSON: видовые места ["Кубань","Панорама"]
    ta_reward_label = db.Column(db.String(50), nullable=True)     # Вознаграждение агенту "4.5-6.5%"
    ta_finishing_main = db.Column(db.Text, nullable=True)         # JSON: доступные отделки [{name,crm_id}]
    ta_finishing_additional = db.Column(db.Text, nullable=True)   # JSON: доп. варианты отделки
    ta_crm_id = db.Column(db.String(50), nullable=True)           # CRM ID квартиры в TA
    ta_north = db.Column(db.Float, nullable=True)                 # Ориентация фасада (градусы от севера)
    ta_status_crm_id = db.Column(db.Integer, nullable=True)       # CRM ID статуса (0=свободная, 23=под запрос)
    ta_exclusive = db.Column(db.Boolean, nullable=True)           # Эксклюзивная квартира
    ta_block_guid = db.Column(db.String(200), nullable=True)      # GUID ЖК (block_guid) для прямой ссылки TA
    end_build_year = db.Column(db.Integer, nullable=True)         # Год сдачи корпуса
    end_build_quarter = db.Column(db.Integer, nullable=True)      # Квартал сдачи корпуса
    ...
    @property
    def cian_address(self):
        """ЦИАН-стиль: Краснодарский край → Краснодар → Прикубанский → ул. Конгрессная, 43к1"""
        parts = []
        try:
            if self.city and self.city.region:
                parts.append(self.city.region.name)
...
class Deal(db.Model):
    ...
    def can_edit(self, user_id, is_manager=False, is_rop=False):
        if self.is_locked:
            return False
        if is_rop:
            return True
...
class PromoBanner(db.Model):
    ...
    overlay_opacity = db.Column(db.Float, default=0.0, nullable=True)
    created_at = db.Column(db.DateTime, default=datetime.utcnow, nullable=False)
    updated_at = db.Column(db.DateTime, default=datetime.utcnow, onupdate=datetime.utcnow, nullable=False)
...
class PartnerManagerRequest(db.Model):
    __tablename__ = 'partner_manager_requests'
    __table_args__ = {'extend_existing': True}
    ...
    partner = db.relationship('Partner', backref='manager_requests')

    def __repr__(self):
        return f'<PartnerManagerRequest partner_id={self.partner_id} status={self.status}>'
...
class TelegramPromotion(db.Model):
    """Горячие акции застройщиков, спарсенные из Telegram-группы"""
    __tablename__ = 'telegram_promotions'
    __table_args__ = {'extend_existing': True}

    id = db.Column(db.Integer, primary_key=True)
    # Привязка к ЖК и застройщику
    residential_complex_id = db.Column(db.Integer, db.ForeignKey('residential_complexes.id'), nullable=True, index=True)
    developer_id = db.Column(db.Integer, db.ForeignKey('developers.id'), nullable=True, index=True)
    # Данные из Telegram
    tg_message_id = db.Column(db.BigInteger, nullable=True, unique=True, index=True)
    tg_thread_id = db.Column(db.Integer, nullable=True, index=True)
    tg_thread_title = db.Column(db.String(500), nullable=True)  # Название ветки (топика)
    tg_chat_id = db.Column(db.BigInteger, nullable=True)
    tg_sender_name = db.Column(db.String(200), nullable=True)
    # Контент акции
    title = db.Column(db.String(500), nullable=True)
    description = db.Column(db.Text, nullable=True)
    raw_text = db.Column(db.Text, nullable=True)
    # Фото (JSON-список путей)
    photos = db.Column(db.Text, nullable=True)  # JSON list of local paths
    # Мета
    is_active = db.Column(db.Boolean, default=True, index=True)
    is_matched = db.Column(db.Boolean, default=False)  # Удалось ли сопоставить с ЖК
    posted_at = db.Column(db.DateTime, nullable=True)   # Дата поста в TG
    parsed_at = db.Column(db.DateTime, default=datetime.utcnow)
    updated_at = db.Column(db.DateTime, default=datetime.utcnow, onupdate=datetime.utcnow)

    complex = db.relationship('ResidentialComplex', backref=db.backref('tg_promotions', lazy='dynamic'))
    developer = db.relationship('Developer', backref=db.backref('tg_promotions', lazy='dynamic'))
    ...
    def __repr__(self):
        return f'<TelegramPromotion {self.id} complex={self.residential_complex_id} thread={self.tg_thread_title}>'
